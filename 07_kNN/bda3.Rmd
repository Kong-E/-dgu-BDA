...
train.norm.df.x <- train.norm.df[, 1:8]; train.norm.df.y <- train.norm.df[, 9]
valid.norm.df.x <- valid.norm.df[, 1:8]; valid.norm.df.y <- valid.norm.df[, 9]
test.norm.df.x <- test.norm.df[, 1:8]; test.norm.df.y <- test.norm.df[, 9]
...
for(i in 1:460) {
  knn.pred <- knn(train = train.norm.df.x, 
                  ...
                  cl = train.norm.df.y,
                  k = i)
  accuracy.df[i,2] <- confusionMatrix(knn.pred, as.factor(valid.norm.df.y))$overall[1]
}
accuracy.df
...
result
accuracy <- confusionMatrix(knn.pred,as.factor(test.norm.df.y))$overall[1]
accuracy
```